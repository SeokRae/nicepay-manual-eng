...
| `orderId`        | String    | Conditional | 64    | Your unique order ID <br>-In case of partial cancellation, orderId cannot be reusable<br>Required when `isNetCancel` is absent or `false` |
| `cancelAmt`      | Int       | No          | 12    | Cancellation Amount<br>If the value is missing, full cancellation will be occured |
...
| `isNetCancel`    | Boolean   | No          | 5     | `true` sends this as a net cancel, the reversal to use when your server never received the authorization result. See the callout below. Default `false` |
| `taxFreeAmt`     | Int       | No          | 12    | Tax-free amount among cancellation amount |
