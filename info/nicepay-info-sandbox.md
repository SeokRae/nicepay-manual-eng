...
1. **Register a token** with [`POST /v1/subscribe/regist`](../api/nicepay-api-billing.md#create-tokenbid-request-parameter). Sandbox decrypts `encData` and checks that the fields your authentication type requires are present and not empty, as in [encData Field Details](../api/nicepay-api-billing.md#encdata-field-details): a missing field fails with [`U317`](../code/nicepay-code.md#api-response-code), and `encData` that it cannot decrypt fails with [`F101`](../code/nicepay-code.md#api-response-code). Sandbox does not check the card number itself or whether the expiry date is in the future, and there is no list of test card numbers. It does not store the card you send: the response has a new `bid` and `cardCode` `04` whatever card you sent.
